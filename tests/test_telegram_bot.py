...
async def test_start(bot):
    ...
    update.message.reply_html.assert_called_once_with(
        "Heyhoy user_1234, Welcome in our service for USC sports. To start, I need some info from you. "
        "What login method would you like to use? You can try (uva)"
    )
...
async def test_ask_sports_valid_method(bot):
    ...
    print(bot.db.insert_user.call_args_list)
    bot.db.insert_user.assert_called_once_with(1234, ANY, 'uva')
    update.message.reply_html.assert_called_once_with(
        "Allright, we registered uva now we need to know the sport you want to participate in. Note that spelling (including captial letters) is very important here as this is where we will search for."
    )
    assert result == SPORT
...
async def test_ask_username(bot):
    ...
    bot.db.edit_data_point.assert_called_once_with(
...
async def test_ask_password(bot):
    ...
    # 1. Verify that db.edit_data_point is called correctly
    bot.db.edit_data_point.assert_called_once_with(
...
async def test_finish_sign_up(mock_encryptor, bot):
    ...
    # 3. Verify that db.edit_data_point is called with the correct arguments
    bot.db.edit_data_point.assert_called_once_with(
        123456,  # Telegram user ID
    ...
    update.message.reply_html.assert_called_once_with(
        "You have finished the sign up process. Many love from us and we hope to see you in the gym! &#10084;"
    )
...
async def test_cancel_setup(bot):
    ...
    update.message.reply_text.assert_called_once_with("Canceled signup, Type /start to start over")
...
async def test_help_command(bot):
    ...
    update.message.reply_text.assert_called_once_with(
        "We'll send you updates on all the trainings. You can sign up via the buttons. To stop, use the /cancel command"
    )
...
async def test_error_handler_login_method_error(mock_info, mock_error, bot):
    ...
    mock_error.assert_called_once_with("An error occured: %s: %s", "Login method is not known", ANY)

    # Assert that the correct message is sent to the user
    update.message.reply_text.assert_called_once_with(
        "Sorry, that login method has not been implemented yet, please choose one from the given list (uva)"
    )
...
async def test_error_handler_already_registered(mock_info, bot):
    ...
    update = MagicMock()
    context = MagicMock()
    context.error.__str__.return_value = 'Message: no such element: Unable to locate element: {"method":"css selector","selector":"button[data-test-id="bookable-slot-book-b'
...
@patch("logging.error")
@patch("logging.info")
async def test_error_handler_callback_query(mock_info, mock_error, bot):
...
@patch("logging.error")
@patch("logging.info")
async def test_error_handler_generic_message_error(mock_info, mock_error, bot):
...
async def test_message_handler_yes_choice(mock_usc_interface, bot):
    ...
    # Mock db behavior
    bot.db.get_lesson_data_by_key = MagicMock(return_value={"sport": "Basketball", "datetime": "2024-09-30 10:00:00"})
    bot.db.get_user = MagicMock(return_value={"username": "user123", "password": "password", "login_method": "uva"})
    bot.db.edit_data_point = MagicMock()

    # Call the message_handler function
    await bot.message_handler(update, MagicMock())

    # Assertions:
    # 1. Ensure the db methods were called with correct arguments
    bot.db.get_lesson_data_by_key.assert_called_once_with("some_key")
    bot.db.edit_data_point.assert_called_once_with("some_key", "response", "Y")
    ...
    mock_usc_interface.return_value.sign_up_for_lesson.assert_called_once_with("Basketball", "2024-09-30 10:00:00")
...
async def test_message_handler_no_choice(mock_interface, bot):
    ...
    # 1. Ensure the db methods were called with correct arguments
    bot.db.get_lesson_data_by_key.assert_called_once_with("some_key")
    bot.db.edit_data_point.assert_called_once_with("some_key", "response", "N")
