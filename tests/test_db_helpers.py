...
def mock_db():
    ...
    with patch('usc_sign_in_bot.db_helpers.psycopg2.connect') as mock_connect, \
         patch('usc_sign_in_bot.db_helpers.Encryptor') as mock_encryptor:
...
def test_insert_user(mock_db):
    ...
    mock_db.cursor.execute.assert_called_once_with(
        ANY,
        ("hashed_value", sign_up_time.isoformat(), "telegram", "123456789")
    )
...
def test_add_to_data(mock_db):
    ...
    # Call the method
    lesson_key = mock_db.add_to_data("Fencing", lesson_time, "user_123", True, trainer="John Doe")

    # Assert that the insert query was executed with correct parameters
    mock_db.cursor.execute.assert_called_once_with(
        ANY,
        ("hashed_value", "user_123", str(lesson_time), "Fencing", "John Doe", True, None)
    )
...
def test_has_received_update(mock_db):
    ...
    mock_db.cursor.execute.assert_called_once_with(
        ANY,
        ("Fencing", str(lesson_time), True, "user_123")
    )
...
def test_get_lesson_data_by_key(mock_db):
    ...
    mock_db.cursor.execute = MagicMock()
    mock_db.cursor.fetchone = MagicMock(return_value=('lesson_123', 'user_123', '2024-09-17T18:00:00', 'Fencing', 'John Doe', True, None))
    mock_db.cursor.description = [('lesson_id',), ('user_id',), ('datetime',), ('sport',), ('trainer',), ('message_sent',), ('response',)]
...
def test_get_user_success(mock_db):
    ...
    mock_db.cursor.fetchone.return_value = ('user123', 'test_user', 'encrypted_password')
...
def test_get_user_custom_query_key(mock_db):
    ...
    mock_db.cursor.fetchone.return_value = ('user@example.com', 'test_user', 'encrypted_password')
