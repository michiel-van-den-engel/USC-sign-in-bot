...
def mock_browser():
    ...
    with patch('usc_sign_in_bot.usc_interface.webdriver.Chrome', new=MagicMock) as MockChrome:
...
def usc_interface(mock_browser):
    ...
    with patch.object(UscInterface, '_login', return_value=None) as mock_login, \
         patch.object(UscInterface, '_set_browser_timezone', return_value=None):
...
def test_constructor_calls_login_and_set_timezone(mock_browser):
    ...
    with patch.object(UscInterface, '_login', return_value=None) as mock_login, \
         patch.object(UscInterface, '_set_browser_timezone', return_value=None) as mock_set_timezone:
...
def test_login_with_uva(usc_interface):
    ...
    with patch.object(usc_interface, '_select_element', side_effect=MagicMock()) as mock_select:
...
def test_set_browser_timezone(usc_interface):
    ...
    with patch.object(usc_interface, 'execute_cdp_cmd') as mock_execute_cmd:
        ...
        mock_execute_cmd.assert_called_once_with('Emulation.setTimezoneOverride', {'timezoneId': 'Europe/Amsterdam'})

def test_click_and_find_element(usc_interface):
    """Test clicking an element using JavaScript."""
    with patch.object(usc_interface, 'execute_script') as mock_execute_script, \
         patch.object(usc_interface, '_select_element', return_value=MagicMock()) as mock_select_element:
...
def test_filter_for_sport(usc_interface, mock_browser):
    ...
    with patch.object(usc_interface, '_select_element', side_effect=MagicMock()) as mock_select_element, \
         patch.object(usc_interface, 'execute_script') as mock_execute_script:
        ...
        mock_select_element.assert_any_call('i[class="fas text-primary fa-chevron-down"]')
        mock_select_element.assert_any_call('//li[label[text()="Football"]]', select_with=By.XPATH)
        dropdown_mock.click.assert_called()
        mock_execute_script.assert_called_once_with("arguments[0].click();", input_element_mock)
...
def test_filter_webelements(usc_interface):
    ...
    # Call the method
    filtered_elements = usc_interface._filter_webelements(elements, "//div[contains(@class, 'desired')]")
    ...
    element1.find_element.assert_called_once_with(By.XPATH, "//div[contains(@class, 'desired')]")
    element2.find_element.assert_called_once_with(By.XPATH, "//div[contains(@class, 'desired')]")
    element3.find_element.assert_called_once_with(By.XPATH, "//div[contains(@class, 'desired')]")
...
def test_extract_info_from_timeslot(mock_sleep, usc_interface):
    ...
    assert result['time'] == dt.combine(dt.now().date(), dt.strptime("12:30", "%H:%M").time())
    assert result['trainer'] == "John Doe"

    slot.find_element.assert_any_call(By.CSS_SELECTOR, 'p[data-test-id="bookable-slot-start-time"] > strong')
    slot.find_element.assert_any_call(By.CSS_SELECTOR, 'span[data-test-id="bookable-slot-supervisor-first-name"]')
...
def test_extract_info_from_timeslot_error_handling(usc_interface, mock_browser):
    ...
    with pytest.raises(ValueError),\
        patch('usc_sign_in_bot.usc_interface.logger') as mock_logger:  # Mock logger for error handling
        result = usc_interface._extract_info_from_timeslot(slot, day_ahead=1)

        assert mock_logger.error.called
        mock_logger.error.assert_called_with(f"Time extraction Failed for {slot}")
        assert result['time'] is not None  # Even if time extraction fails, a datetime should still be returned

def test_loop_over_the_days(usc_interface):
    """Test looping over the days to apply a function."""
    with patch.object(usc_interface, '_select_all_elements', return_value=[MagicMock() for _ in range(3)]) as mock_select_elements, \
         patch.object(usc_interface, '_click_and_find_element'), \
         patch.object(usc_interface, '_filter_webelements', return_value=[MagicMock() for _ in range(2)]) as mock_filter_elements, \
         patch.object(usc_interface, 'execute_script'), \
         patch.object(usc_interface, 'reset_driver'):

        # Define a mock function to apply to each slot
        mock_function = MagicMock(return_value="result")

        result = usc_interface._loop_over_the_days(target_days=2, sport="Football", function_to_do=mock_function)
...
def test_select_day(usc_interface, weekdays):
    ...
    with patch.object(usc_interface, '_select_all_elements', return_value=[MagicMock() for _ in range(5)]) as mock_select_elements, \
         patch.object(usc_interface, '_filter_webelements', side_effect=[[MagicMock()], []]) as mock_filter_elements, \
         patch.object(usc_interface, 'execute_script'):

        usc_interface._select_day(date)

        # Verify that the method filtered for the correct date string
        mock_filter_elements.assert_any_call(ANY, f'.//*[contains(text(), "{date_str}")]')
...
def test_click_bookable_right_course(usc_interface):
    ...
    with patch.object(usc_interface, '_select_all_elements', return_value=[MagicMock() for _ in range(3)]) as mock_select_elements, \
         patch.object(usc_interface, '_filter_webelements', return_value=[MagicMock()]) as mock_filter_webelements, \
         patch.object(usc_interface, 'execute_script') as mock_execute_script:
        ...
        mock_select_elements.assert_called_once_with('div[data-test-id="bookable-slot-list"]')
        mock_filter_webelements.assert_called_once_with(ANY, f"*[contains(., '{sport}') and contains(., '{date.strftime('%H:%M')}')]")
...
def test_click_sign_on(usc_interface):
    ...
    with patch.object(usc_interface, '_select_element', side_effect=[MagicMock(), MagicMock()]) as mock_select_element, \
         patch.object(usc_interface, 'execute_script') as mock_execute_script:
        ...
        mock_select_element.assert_any_call('button[data-test-id="details-book-button"]')
...
def test_reset_driver(usc_interface):
    ...
    with patch.object(usc_interface, '_select_all_elements', return_value=[MagicMock() for _ in range(3)]) as mock_select_all_elements, \
         patch.object(usc_interface, '_filter_webelements', side_effect=[[], [MagicMock()]]) as mock_filter_webelements, \
         patch.object(usc_interface, '_select_element') as mock_select_element:

        usc_interface.reset_driver()

        # The function should loop until it finds 'Vandaag'
        mock_select_all_elements.assert_called_with('a[data-test-id-day-selector="day-selector"]')
        mock_filter_webelements.assert_called_with(ANY, "//*[contains(text(), 'Vandaag')]")
        mock_select_element.assert_called_with('//i[@class="fa fa-chevron-left"]/..', select_with=By.XPATH)
...
def test_sign_up_for_lesson(usc_interface):
    ...
    with patch.object(usc_interface, '_filter_for_sport') as mock_filter_for_sport, \
         patch.object(usc_interface, '_select_day') as mock_select_day, \
         patch.object(usc_interface, '_click_bookable_right_course') as mock_click_bookable_right_course, \
         patch.object(usc_interface, '_click_sign_on') as mock_click_sign_on, \
...
def test_sign_up_for_lesson_with_exception(usc_interface):
    ...
    with patch.object(usc_interface, '_filter_for_sport', side_effect=Exception("Test exception")), \
         patch.object(usc_interface, 'reset_driver') as mock_reset_driver:
...
def test_get_all_lessons(usc_interface):
    ...
    with patch.object(usc_interface, '_filter_for_sport') as mock_filter_for_sport, \
         patch.object(usc_interface, '_loop_over_the_days', return_value=["lesson1", "lesson2"]) as mock_loop_over_the_days, \
         patch.object(usc_interface, 'reset_driver') as mock_reset_driver:
        ...
        mock_filter_for_sport.assert_called_once_with(sport)
        mock_loop_over_the_days.assert_called_once_with(days_in_future, sport, usc_interface._extract_info_from_timeslot)
        mock_reset_driver.assert_called_once()
